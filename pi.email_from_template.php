...
class Email_from_template {
	...
	function __construct($str = '') { $this->init($str); }

	function Email_from_template($str = '') { $this->init($str); }
	...
	function init($str = '')
	{
		...
		$from = (($from = $this->EE->TMPL->fetch_param('from')) === FALSE) ? $this->from : ee('Security/XSS')->clean($from);
		$to = (($to = $this->EE->TMPL->fetch_param('to')) === FALSE) ? $this->to : ee('Security/XSS')->clean($to);
		$cc = (($cc = $this->EE->TMPL->fetch_param('cc')) === FALSE) ? FALSE : ee('Security/XSS')->clean($cc);
		$bcc = (($bcc = $this->EE->TMPL->fetch_param('bcc')) === FALSE) ? FALSE : ee('Security/XSS')->clean($bcc);

		$subject = (($subject = $this->EE->TMPL->fetch_param('subject')) === FALSE) ? $this->subject : $subject;
		$alt_message = (($alt_message = $this->EE->TMPL->fetch_param('alt_message')) === FALSE) ? FALSE : ee('Security/XSS')->clean($alt_message);
		...
		$attachments = (($attachments = $this->EE->TMPL->fetch_param('attachments')) === FALSE) ? FALSE : ee('Security/XSS')->clean($attachments);
    ...
    static function usage() { return file_get_contents(__DIR__.'/README.md'); } // END usage()
